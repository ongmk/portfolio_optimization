...
def maxSR(meanReturns, covMatrix, riskFreeRate=0, constraintSet=(0, 1)):
    ...
    result = sc.minimize(
        negativeSR,
        [1.0 / numAssets for i in range(numAssets)],
        args=args,
        method="SLSQP",
        bounds=bounds,
...
def efficientOpt(meanReturns, covMatrix, returnTarget, constraintSet=(0, 1)):
    ...
    constraints = (
        {
            "type": "eq",
            "fun": lambda x: portfolioReturn(x, meanReturns, covMatrix) - returnTarget,
        },
...
def calculatedResults(meanReturns, covMatrix, riskFreeRate=0, constraintSet=(0, 1)):
    ...
    minVol_returns, minVol_std = round(minVol_returns * 100, 2), round(
        minVol_std * 100, 2
    )
    minVol_allocation = pd.DataFrame(
        minVol_Portfolio["x"], index=meanReturns.index, columns=["allocation"]
    )
